...
public class PaymentInitiationServiceImplTest {
	...
	public void givenValidRequest_WhenMethodIsCalled_ThenReturnsSuccessfulResponse() throws IOException{
		...
		PaymentInitiationResponseOverview responseOverview = service.validateRequest(encodedCertificate.trim(), encodedSignature.trim(), request);
	...
	private void perpareDataForSuccesfulResponse() throws IOException{
		...
		Mockito.doNothing().when(validator).validateRequest(Mockito.anyObject(), Mockito.anyString(), Mockito.anyObject());
