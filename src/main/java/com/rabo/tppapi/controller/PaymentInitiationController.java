...
public class PaymentInitiationController {
	...
	/**
	 * This method receives the httpServlet request and request Body for processing
	 * @param servletRequest servlet request object
	 * @param request request body 
	 * @return PaymentInitiationResponse
	 */
	@PostMapping(value = "/v1.0.0/initiate-payment")
	public ResponseEntity<PaymentInitiationResponse> paymentInitiationRequest(HttpServletRequest servletRequest,@Valid @RequestBody final PaymentInitiationRequest request){ 
		String signature = servletRequest.getHeader("Signature");
		String certificate = servletRequest.getHeader("Signature-Certificate");
		PaymentInitiationResponseOverview responseOverview = tppAPIService.validateRequest(certificate,signature,request);
